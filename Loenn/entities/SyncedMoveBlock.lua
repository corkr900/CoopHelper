...
local syncedMoveBlock = {}

local moveBlockDirections = {
    "Up", "Down", "Left", "Right"
}

syncedMoveBlock.name = "corkr900CoopHelper/SyncedMoveBlock"
syncedMoveBlock.depth = 8995
syncedMoveBlock.minimumSize = {16, 16}
syncedMoveBlock.fieldInformation = {
    direction = {
        options = moveBlockDirections,
        editable = false
    }
}
syncedMoveBlock.placements = {}
...
for _, direction in ipairs(moveBlockDirections) do
    for steerable = 1, 2 do
		...
		table.insert(syncedMoveBlock.placements, {
			name = name,
			data = {
				width = 16,
				height = 16,
...
function syncedMoveBlock.sprite(room, entity)
